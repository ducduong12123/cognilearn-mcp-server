...
- **Stateless streamable HTTP.** `FastMCP(stateless_http=True)` mounted on a FastAPI app at
  `/mcp`, so it runs behind ordinary load balancers and scales horizontally.
...
# → http://127.0.0.1:8002/healthz   and   MCP endpoint http://127.0.0.1:8002/mcp
```
...
```json
{
  "mcpServers": {
    "cognilearn-memory": {
      "command": "npx",
      "args": ["-y", "mcp-remote", "http://127.0.0.1:8002/mcp",
...
**n8n** — *MCP Client* node, endpoint `https://<host>/mcp`, transport *HTTP Streamable*; pass the
